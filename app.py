...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "ضع_التوكن_هنا_أو_اتركه_يقرأ_من_البيئة")
...
def init_database():
    ...
    cursor = conn.cursor()
    cursor.execute("""CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY, topic_id INTEGER UNIQUE NOT NULL, first_name TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""")
    cursor.execute("""CREATE TABLE IF NOT EXISTS topics (topic_id INTEGER PRIMARY KEY, user_id INTEGER NOT NULL, first_name TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""")
    conn.commit()
    conn.close()
...
async def start(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(f"👋 مرحباً بك في صفور، {user.first_name}!\n\nأرسل رسالتك وسيتم تحويلها إلى فريق الدعم.")
...
async def handle_group_reply(update: Update, context: CallbackContext):
    ...
    if not reply_text:
        return
    try:
        await context.bot.send_message(chat_id=user_id, text=f"📩 <b>رد من فريق الدعم</b>\n\n{reply_text}", parse_mode="HTML")
...
def run_flask():
    app_flask.run(host='0.0.0.0', port=int(os.environ.get('PORT', 5000)), debug=False, use_reloader=False)

def main():
    init_database()
    flask_thread = Thread(target=run_flask, daemon=True)
    flask_thread.start()
    print("🌐 خادم Flask يعمل في الخلفية...")

    application = Application.builder().token(BOT_TOKEN).build()
    application.add_handler(CommandHandler("start", start))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.ChatType.PRIVATE, handle_message))
    application.add_handler(MessageHandler(filters.TEXT & filters.Chat(OPERATOR_GROUP_ID), handle_group_reply))
